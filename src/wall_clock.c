...
// 每天只重新對時一次：RP2040 內建震盪器的漂移量級是一天幾秒，不需要每次
// 上傳都重打一次 NTP（那樣每次上傳都多一次網路來回，卻幾乎沒有實質收穫）。
// 開機後第一次呼叫（從沒成功校時過）一定會真的去打，之後只有距離上次成功
// 校時超過這個間隔才會真的重新查詢，其餘時間直接沿用已經校過的基準點。
#define NTP_RESYNC_INTERVAL_MS (24ULL * 60 * 60 * 1000)

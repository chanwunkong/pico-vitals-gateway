...
**2026-09-24 重新加回事件觸發自動同步，取代單一 22:00 定時視窗的每日涵蓋率不足問題**：22:00 定時觸發只能可靠涵蓋「22:00 前 27 分鐘內」量測過的裝置（見 27 分鐘自動關機的實測），如果病患不是固定睡前量測，白天量的資料要等到隔天甚至更久才會被 backfill 補上。根因分析：2026-09-02 拿掉「掃到廣播就自動連線」的舊設計，是因為當時冷卻時間只有 60 秒、遠比裝置清醒時間（~27 分鐘）短，導致裝置清醒的整段時間內每 60 秒就被重新連線一次、響一次，不是「事件觸發」這個機制本身有問題。**修法**：`mode_ble_receive.c` 新增「第一次掃到 GM700SB 廣播就排定一個延遲觸發」（`s_rightest_auto_trigger_pending`/`s_rightest_auto_trigger_at`，延遲時間 `RIGHTEST_AUTO_TRIGGER_DELAY_MS`，**使用者 2026-09-24 決定暫時設 0**，之後想留時間給病患收好裝置再連線的話只要調大這個數字），延遲時間到了就開跟 KEY1 短按完全一樣的同步視窗（`s_rightest_manual_sync_active`）；同時把 `DEVICE_RECONNECT_COOLDOWN_MS[FORA_DEVICE_RIGHTEST_GM700SB]` 從 60 秒拉長到**使用者 2026-09-24 決定的 1 小時**，蓋過實測到的 27 分鐘清醒時間，確保同一次清醒期間只會觸發一次、只響一聲。KEY1 手動觸發／每天 22:00 定時觸發兩條路徑原封不動保留當備援，三者共用同一個視窗機制，互不衝突。~~已編譯驗證通過（RP2040/Pico 2 W 皆過關），還沒有實機測試~~

**2026-09-24 交付 11 台之後，實機測到一個當時沒發現的 bug：KEY1 手動觸發被自己新加的 1 小時冷卻擋住**。「三者共用同一個視窗機制」這句話本身沒錯，但視窗開啟後真正要連線那一步，原本的程式碼不分來源地套用了同一組 `s_kind_cooldown_until[kind]` 冷卻檢查（這段邏輯在事件觸發功能之前就存在，是給 IR42/O2/D40/MD6 這種沒有視窗機制、純粹靠冷卻節流的裝置用的）。冷卻時間從 60 秒拉長到 1 小時之後，這組共用檢查變成連 KEY1／22:00 開的視窗也一起被擋——而且冷卻是**整個裝置種類共用一組，不是分裝置位址各自算**，只要有任一台 GM700SB 觸發過一次連線（不管成功失敗），接下來一小時內**所有**GM700SB（含從沒連過的全新裝置）都會一起連不上。實機 log 佐證：裝置訊號在整段時間內清楚穩定，`manual sync window expired without finding the device` 卻還是跳出來。

**已修正（`mode_ble_receive.c` `handle_advertising_report()`）**：GM700SB 在視窗已開的情況下跳過這組共用冷卻檢查（視窗本身已經是獨立的節流關卡：KEY1／22:00 是刻意觸發、事件自動觸發自己在排定那一步已經檢查過冷卻），其他裝置不受影響。**這個修正在交付的 11 台上沒有生效**（燒錄之後才發現問題，見 commit `efaea04`／tag `delivery-2026-09-24` 是實際交付的版本，這個修正是後續 commit），下一輪韌體更新才會佈署到已交付的機器。**已交付機器的暫時因應**：冷卻計時只存在 RAM，斷電重開機即可清除卡住的狀態，KEY1 會恢復正常，直到下次又有裝置自動同步觸發才會再卡住一小時（但不影響核心的事件觸發自動同步，那條路徑不會被這個 bug 擋住）。已編譯驗證通過（RP2040/Pico 2 W 皆過關），這個修正本身還沒有實機測試過。

**還沒實機驗證的原始三項**（KEY1 修正之外，事件觸發自動同步本身）：(1) 病患量測後，Gateway 真的會在延遲時間到（目前設 0，應該幾乎立刻）觸發連線並響一聲；(2) 同一次清醒期間（~27 分鐘內）不會被重複觸發第二次；(3) 冷卻期過後、裝置下一次真的被使用時，能正常再次觸發，不會被卡死。
